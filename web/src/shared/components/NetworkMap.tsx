...
function createMarkerElement(probe: Probe, active: boolean, mode: 'fleet' | 'detail', onSelect?: (probeId: string) => void) {
  ...
  Object.assign(squareEl.style, {
    ...
    boxShadow: active
      ? '0 0 0 2px rgba(255, 106, 0, 1), 0 0 24px 8px rgba(255, 106, 0, 0.72)'
      : '0 0 14px 4px rgba(255, 106, 0, 0.5)',
  })
...
export function NetworkMap({ probes, selectedId, onSelect, mode = 'fleet', className }: NetworkMapProps) {
  ...
  useEffect(() => {
    ...
    if (map.loaded()) {
      ...
      map.once('load', focusSelectedProbe)
    }

    return () => map.off('load', focusSelectedProbe)
  ...
  useEffect(() => {
    ...
    if (map.loaded()) {
      ...
      map.once('load', fitFleetBounds)
    }

    return () => map.off('load', fitFleetBounds)
