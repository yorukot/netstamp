import { type FormEvent, type KeyboardEvent as ReactKeyboardEvent, type MouseEvent, useEffect, useRef, useState } from 'react'
          ...
          <div className={styles.workflowTrack} style={{ transform: `translateX(-${currentStep * 100}%)` }}>
            ...
            <section className={styles.workflowPanel} aria-hidden={currentStep !== 2}>
              ...
              <div className={styles.formGrid}>
                <TextField label="Location (optional)" value={probeLocation} placeholder="Taipei, Taiwan" disabled={currentStep !== 2} onChange={(event) => setProbeLocation(event.currentTarget.value)} />
              ...
              <div className={styles.tagPicker}>
                ...
                <div className={styles.tagCloud}>
                  {tagOptions.map((tag) => (
                    <button className={`${styles.tagButton} ${selectedTags.includes(tag) ? styles.tagSelected : ''}`} key={tag} type="button" disabled={currentStep !== 2} onClick={() => toggleTag(tag)}>
                      {tag}
                    </button>
                  ))}
                </div>
