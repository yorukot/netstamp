...
export function InsightPage() {
  ...
  const pickerOptions = view === 'probe'
    ? probes.map((probe) => ({ value: probe.id, label: `${probe.name} · ${probe.location}` }))
    : checks.map((check) => ({ value: check.id, label: `${check.target} · ${check.type}` }))

  const graphCards: GraphCard[] = view === 'probe'
  ...
  return (
    <section className={styles.screen}>
      <ScreenHeader
        ...
        title="Insight"
        copy="Pick a time window, then switch between probe-first and target-first views to compare every matching measurement graph."
      />
