...
- 版本化表之間的 `probe_id`、`check_id` 是 logical reference；service layer 需要驗證它們在同一個 effective config version 中存在。
- `config_version` 只唯一識別全域設定版本，不唯一識別版本化表中的單一設定 row。單一版本化設定 row 由 logical `id` 與 `config_version` 一起識別。
...
| `change_type` | `TEXT` | `NOT NULL` | 例如 `create_probe`、`update_check`、`delete_check` |
...
`checks` 表示可被 probe 執行的一個量測定義。第一版直接把原本的 target 與 check 合併在這張表，避免為了簡單檢查維護過多中介表。`target` 是這個 check 的主要目標，不是 foreign key。
...
| 欄位 | PostgreSQL type | 限制 | 說明 |
| --- | --- | --- | --- |
| `id` | `UUID` | `NOT NULL` | Logical probe-check ID |
| `config_version` | `BIGINT` | `NOT NULL REFERENCES config_versions(config_version)` | 此 row 所屬設定版本 |
| `probe_id` | `UUID` | `NOT NULL` | Logical probe ID |
| `check_id` | `UUID` | `NOT NULL` | Logical check ID |
...
| `probe_check_id` | `UUID` | `NOT NULL` | 當時執行的 logical probe-check ID |
| `check_id` | `UUID` | `NOT NULL` | 當時執行的 logical check ID |
| `probe_id` | `UUID` | `NOT NULL` | Logical probe ID |
| `check_type` | `TEXT` | `NOT NULL` | `ping`、`traceroute`、`dns` |
| `target` | `TEXT` | `NOT NULL` | 當時執行的目標快照 |
| `target_type` | `TEXT` | `NOT NULL` | 當時執行的目標類型快照 |
...
- `results (probe_check_id, started_at DESC)`
...
- 從指定 `config_version` 下有效的 `probes`、`probe_checks`，以及 `checks` 建立每個 probe 專屬的設定快照。
...
- 在本機加入 jitter，避免同步量測。
- 串流包含 `probe_check_id`、`check_id`、`target`、`check_type`，以及 `config_version` 的結果。
