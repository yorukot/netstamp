...
export function DataTable<Row extends object>({ columns, rows, getRowKey, onRowClick, selectedKey, emptyLabel = "No results" }: DataTableProps<Row>) {
	return (
				...
				<tbody>
					{rows.length ? (
						rows.map((row, index) => {
							...
							return (
								<tr key={rowKey} className={selected ? styles.selected : undefined} onClick={onRowClick ? () => onRowClick(row) : undefined}>
									{columns.map(column => (
